...
const ModalMenu = ({ active, setActive, editions, item }) => {
  let [priceItem, setPriceItem] = useState(item.price || 0)
  let editionsTest = () => {
      ...
      <div className={style.modal__block}>
        ...
        <div className={style.modal__content}>
          ...
          <div className={style.modal__item}>
            {editions !== undefined &&
              editions.map((item) => (
                <ItemEditions key={item.uid} item={item} selectDiff={selectDiff}/>
              ))}
          </div>
        </div>
        <div className={style.modal__content_main}></div>
        <div className={style.modal__blockPrice}>
          <div className={style.modal__blockPrice_price}>Итог: {priceItem}₽</div>

          <button className={style.modal__blockPrice_btn}>
            Добавить в заказ
          </button>
        </div>
      </div>
